...
//The array to hold x/y-coordinates of the food
short food[2];
...
genAgain:
    ...
    food[0] = randomRange(0, WIDTH_AREA);
    //Generate food for the y-axis
    food[1] = randomRange(5, HEIGHT_AREA);
    Node *current = head;
...
void setup() {
    ...
    // Create the snake
    for (int i = 0; i < 8; i++) {
        addNode((WIDTH_AREA / 2) - i, HEIGHT_AREA / 2);
    }
...
void loop() {
    ...
    DispUpdate();
    sleep_ms(70 - ((unsigned long)current_time_ms - begin_time));

    if (head->x < 0 || head->x >= WIDTH_AREA || head->y < SCORE_AREA) {
        alive = 0;
    }
