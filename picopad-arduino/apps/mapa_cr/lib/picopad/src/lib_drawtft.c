...
void DrawRectShadow(int x, int y, int w, int h, u8 shad)
{
	...
	for (; h > 0; h--)
	{
		for (i = w; i > 0; i--)
		{
			c = *d;
			c2 = 0;
			if (shad & B0) c2 = (c >> 4) & ~(B1|PICOB2|B3|B4 | B7|B8|B9|B10 | B12|B13|B14|B15);
			if (shad & B1) c2 += (c >> 3) & ~(PICOB2|B3|B4 | B8|B9|B10 | B13|B14|B15);
			if (shad & PICOB2) c2 += (c >> 2) & ~(B3|B4 | B9|B10 | B14|B15);
...
{
#define VAL6(a) (((a) >> 4) & ~(B1|PICOB2|B3|B4 |B7|B8|B9|B10 |B12|B13|B14|B15))
#define VAL12(a) (((a) >> 3) & ~(PICOB2|B3|B4 |B8|B9|B10 |B13|B14|B15))
