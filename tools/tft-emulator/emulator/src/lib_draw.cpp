...
sf::RenderWindow window(sf::VideoMode( sf::Vector2u{ DispWidth, DispHeight } ), "Picopad SDK TFT emulator");
...
unsigned DispDirtyX1, DispDirtyX2, DispDirtyY1, DispDirtyY2;
...
void DispUpdate()
{
    ...
    {
        // Alokace RGBA bufferu (šířka * výška * 4)
        auto* pixels = new std::uint8_t[(DispDirtyX2 - DispDirtyX1) * (DispDirtyY2 - DispDirtyY1) * 4];

        for (int y = DispDirtyY1; y < DispDirtyY2; ++y)
        {
            for (int x = DispDirtyX1; x < DispDirtyX2; ++x)
            {
                u16* s0 = &FrameBuf[x + y * DispWidth];
                std::size_t pos = (
                    (x - DispDirtyX1)
                  + (y - DispDirtyY1) * (DispDirtyX2 - DispDirtyX1)
                ) * 4;

                pixels[pos + 0] = static_cast<std::uint8_t>((( *s0 >>  8) & 0xF8) | (((*s0 >> 13) & 0x07)));
                pixels[pos + 1] = static_cast<std::uint8_t>((( *s0 >>  3) & 0xFC) | (((*s0 >>  9) & 0x03)));
                pixels[pos + 2] = static_cast<std::uint8_t>((( *s0 <<  3) & 0xF8) | (((*s0 >>  2) & 0x07)));
                pixels[pos + 3] = 255;  // A
            }
        }

        // Aktualizace sub-oblasti textury (SFML 3.0)
        FrameTex.update(pixels,sf::Vector2u{ (DispDirtyX2 - DispDirtyX1), (DispDirtyY2 - DispDirtyY1) }, sf::Vector2u{ DispDirtyX1, DispDirtyY1 });
        delete[] pixels;
